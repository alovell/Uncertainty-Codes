...
!           call flush(6)
	...
	!write(400,*) datalen(id),id, datavals(ip,id)
...
!           WRITE(KO,*) ' XS',datangles(ip,id),THETA,ITH,IANGLS(ip,id)
	      ...
	      wij(ip,:,id) = wij(ip,:,id)*RUTH**2
	      print *, dataerr(ip,id),wij(ip,:,id)
             endif
	      ...
	      wij(ip,:,id) = wij(ip,:,id)*sfactor**2
             endif

            chi = (theory/datanorm-datavals(ip,id))!/dataerr(ip,id)
	    !chi = 0.d0
	    !do ig=1,datalen(id)
	     !  chi = chi + (theory/datanorm-datavals(ip,id))
    !x	       *(theory/datanorm-datavals(ig,id))
    ! x            *wij(ip,ig,id)
    !        print *, theory/datanorm
    !        print *, theory/datanorm-datavals(ip,id),theory/datanorm-datavals(ig,id)
!	    print *, datavals(ip,id),datavals(ig,id)
!	    enddo
	    data_error(ip,id) = chi
!	    print *, ip, id, datalen(id), data_error(ip,id)
	    if(ip==1) then
	      !write(400,*) nvars,srch_value
	      do ivar=1,nvars
	        !write(400,*) ivar,nvars
	   	write(400,282) srch_name(ivar), srch_value(ivar)
282	   	format('# ',a10,' = ',f16.10)
	      enddo
	    endif
	    write(400,*) data_error(ip,id) !,theory/datanorm-datavals(ip,id)
	    !write(400,*) theory/datanorm-datavals(ip+1,id)
281	    format(f14.8)
!	    close(400)
