...
C    ---------
     ...
     & 		HASO(kpmax),CPSO,INCOUL
      ...
      CHARACTER*9 LNL(2)
      CHARACTER*8 POTL(5),POPR(3),REALCM(2),RIC(3)
C
...
875   FORMAT(/' data =',4I3,F8.4, ',  so  < [',A8,' #',I2,'] / [',
     &A8,' #',I2,']  * Eigen-form #',I3,'>  =',F9.4,12X,'BE =',F7.3,I6)
	if(ipa>0) 
...
884   FORMAT(/5X,'So FINITE-RANGE TRANSFER : IP1 =',I2,' (',A5,'), IP2='
     &,I2,' =',A12,',  for',I4,' pairs of bound states summed into',I4,
     & ' pairs')
      IF(QQ.GE.-2) WRITE(KO,8841) NAME(1+ICV,ICV*ICORET+(1-ICV)*ICOREP)
...
8857    continue
!	call flush(48)
      IF(.NOT.COUPLE.OR.ITER.EQ.0 .OR. MINL.GT.MAXL) GO TO 999
      IF(     FFREAL) NQ = 1
      IF(.NOT.FFREAL) NQ = 2
           CALL CHECK(NLO,MAXNLO,9)
...
C                    SOME L-TRANSFERS
     ...
     X	                 I,I*NLL*NLO*NQ*lreal8*1E-6
 8858           FORMAT('0File ',I3,' needs RL =',I8,' ',A9,' numbers',
     X		       ', and NR =',I6,'. i.e.',F7.2,' Megabytes')
