...
C    ----------------------------------
      ...
      integer NAI,PARITYI,LVALI,LVALE,ITI,NRF,ig
...
!#	write(48,*) 'Makeset: NFDEC = ',NFDEC,N,MAXB,NICH,' set'
	...
	if(NFDEC>10 000 000) write(KO,*) 
     ...
     &'Allocate CLIST(',MAXCH,MAXCH,MCLIST,') in ',memuse,' GB' 
      ...
      if (allocated (ecm)) deallocate (ecm)
      if (allocated (xs)) deallocate (xs)
      if (allocated (income)) deallocate (income)
      if (allocated (cfmat)) deallocate (cfmat)
      if (allocated (cgmat)) deallocate (cgmat)
      if (allocated (crcrat)) deallocate (crcrat)
      if (allocated (cutval)) deallocate (cutval)
      if (allocated (blockd)) deallocate (blockd)
      if (allocated (ll1)) deallocate (ll1)
      if (allocated (readwf)) deallocate (readwf)
      if (allocated (lval)) deallocate (lval)
      if (allocated (part)) deallocate (part)
      if (allocated (excit)) deallocate (excit)
      if (allocated (initl)) deallocate (initl)
      if (allocated (jval)) deallocate (jval)
      if (allocated (jproj)) deallocate (jproj)
      if (allocated (jtarg)) deallocate (jtarg)
      if (allocated (exch)) deallocate (exch)
      if (allocated (clist)) deallocate (clist)
      if (allocated (cff)) deallocate (cff)
      if (allocated (nclist)) deallocate (nclist)
      if (allocated (nflist)) deallocate (nflist)
      if (allocated (smat)) deallocate (smat)
      if (allocated (chno)) deallocate (chno)
      if (allocated (ch)) deallocate (ch)
      if (allocated (same)) deallocate (same)
      if (.not.rterms) then
        if (allocated (src)) deallocate (src)
	if (allocated (psi)) deallocate (psi)
	if (allocated (sratio)) deallocate (sratio)
	if (allocated (soma)) deallocate (soma)
	if (allocated (spad)) deallocate (spad)
	if (allocated (ferw)) deallocate (ferw)
	if (allocated (fim)) deallocate (fim)
	if (allocated (fimd)) deallocate (fimd)
	if (allocated (simple)) deallocate (simple)
...
881	    format('@legend string',I3,' "',f8.1,a1,'/',i2,i2,'"')
     ...
     x            .and.PARITY==data_par(id)) then
		 do ip=1,datalen(id)
		   ...
		   do ig=1,datalen(id)
		      EE = (T-datavals(ip,id))*(T-datavals(ig,id))*wij(ip,ig,id)
                      data_chisq(id) = data_chisq(id) + abs(EE)
		      !print *, EE, data_chisq(id)
		   enddo
		   data_error(ip,id) = EE
		   if(ip==1) then
		     	write(400,282) data_ch(id)
282			format('# partition between error sets',f16.10)
		   endif
		   write(400,281) data_error(ip,id)
281		   format(f14.8)
!		   print *, data_error(ip,id)
                   theoryvals(ip,id) = T
		   write(6,*) theoryvals(ip,id)
!                  theoryplot(ip,id) = T
		   endif
	          enddo
		  endif
...
!           WRITE(KO,*) ' dataset ',id,kq1,lq1,real(datanorm)
		 do ip=1,datalen(id)
		   ...
		   if(data_idir(id)==-1) then  ! convert to absolute, first time
		     ...
		     if(ip==1) then
		     	write(400,283) 
283			format('# partition between error sets ')
		     endif
	 	   !EE = (T/datanorm-datavals(ip,id))/dataerr(ip,id)
		   do ig=1,datalen(id)
		      EE = (T/datanorm-datavals(ip,id))*(T/datanorm-datavals(ig,id))
     x                   *wij(ip,ig,id)
                      data_chisq(id) = data_chisq(id) + abs(EE)
		      !print *, EE,data_chisq(id)
		   enddo
		   data_error(ip,id) = EE
		   write(400) data_error(ip,id)
!		   print *, data_error(ip,id)
                   theoryvals(ip,id) = T
		   endif
	          enddo
  ...
  970 if(final) call fkind(written,KO)
	...
	if(OPEN12) then
      ...
      do id=1,datasets
	   ...
	   write(400,284) srch_value(data_par(id))
284	   format('# parameters between error sets ',f20.10)
            t = (srch_value(data_par(id))-datavals(1,id))/dataerr(1,id)
            data_chisq(id) = t**2
	    !print *, t,data_chisq(id)
! this type (6) is different, value and error for search parameter
! only one error for each set (not set of points)	    
	    	data_error(1,id) = t
		write(400,281) data_error
!		print *, data_error(ip,id)
            endif
